...
export default Ember.Component.extend({
	...
	didInsertElement(){
		/*Ember.$('#city-select').change(function(){
			console.info(this.value);
		});*/
		Ember.$('#city_register').val(Ember.$('#city-select option:selected').text())
	
		Ember.$('body').on('change','#city-select', function(e){
			e.stopPropagation();
			Ember.$('#city_register').val(Ember.$('#city-select option:selected').text())

			/*Ember.$.post({
		    	url : Config.site.rajaongkir_url + Config.site.rajaongkir_endpoint + 'cost', 
		  		headers: Config.site.rajaongkir_headers,
		 		data: {
		 			origin: 151,
		 			destination: Ember.$('#city-select').val(),
		 			weight: 1000,
		 			courier: 'jne'
		 		}
		    }).then(data => {
